...
Meteor.methods({
  ...
  'saveFavorite': function (postId) {
    ...
    if(_.contains(thisPost.favorites, Meteor.userId()))
      Posts.update({_id: postId}, {$pull: {favorites: Meteor.userId()}})
    else{
      Posts.upsert({_id:postId}, {$addToSet:{
  ...
  'newComment': function (postId, comment) {
    ...
    if(Meteor.userId() != thisPost.info.postOwner && !isRedundancy(postId,'comment')){
