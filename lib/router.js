...
Router.route('/adminsite',function () {
  if(Meteor.user && Meteor.user().profile.privileged) this.render('admin')
  else this.redirect('/')
},
  {
    name:'admin',fastRender: true,
    onBeforeAction : function () {
      if(Meteor.user().profile.asAdmin.loggedIn) this.redirect('/adminsite/superuser')
      this.next()
    }
  }
)
